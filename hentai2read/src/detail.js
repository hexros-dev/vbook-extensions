...
function execute(url) {
    ...
    for (let i = 2; i <= category_length + 1; i++ ){
        genres.push({
            title: doc.select(`ul.list > li:nth-child(11) > a:nth-child(${i})`).text(),
            input: doc.select(`ul.list > li:nth-child(11) > a:nth-child(${i})`).attr("href"),
            script: "gen.js"
        })
    }
    var detail = `
    ...
    <br>
    Score: ${doc.select(".push-10-t > small").text().replace("(score ", "").replace(")", "")}
    `
    var ongoing = true;
    ...
    for (let i = 2; i <= genres_length + 1; i++){
        var title = doc.select(`ul.list > li:nth-child(12) > a:nth-child(${i})`).text();
        var input = doc.select(`ul.list > li:nth-child(12) > a:nth-child(${i})`).attr("href");
        genres.push({
            title,
            input,
            script: "gen.js"
    ...
    var suggests = [
        {
        title: "Cùng tác giả",
        input: doc.select("#main-container > section > div > div.col-xs-12.col-md-4 > div:nth-child(5) > ul").html(),
        script: "suggest.js"
        },
        {
            title: "Dựa trên tag",
            input: doc.select("#main-container > section > div > div.col-xs-12.col-md-4 > div:nth-child(3) > ul").html(),
            script: "suggest.js"
        }
    ]
